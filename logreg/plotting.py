...
class Example:
    ...
    def __init__(self, label, words, vocab, df):
        ...
        total_words_in_doc = sum(self.x)
        if df is not None:
            self.idf = zeros(len(vocab))
            for word, count in [x.split(":") for x in words]:
                if word in vocab:
                    assert word != kBIAS, "Bias can't actually appear in document"
                    N = 1197
                    nt = df[vocab.index(word)] + 1
                    ft = 1 + float(count) / (total_words_in_doc + 1)
                    ft = ft * N / nt
                    self.idf[vocab.index(word)] += ft * log(1 + (N - nt) / nt, 10)

            self.idf[0] = 1


class LogReg:
    def __init__(self, num_features, mu, step=lambda x: 0.5):
        """
    ...
    def sg_update(self, train_example, iteration, lazy=False, use_tfidf=False):
        """
        ...
        if use_tfidf == True:
            whichVector = train_example.idf
        else:
            whichVector = train_example.x

        if lazy == False:
            pi = sigmoid(numpy.dot(self.beta, whichVector), threshold=20.0)
            self.beta = self.beta * (1 - 2 * self.step(iteration) * self.mu) + self.step(iteration) * (
            train_example.y - pi) * whichVector
            # print(self.beta)
            # return self.beta

        elif lazy == True:
            self.lazy = True
            nonZeroArray = numpy.nonzero(whichVector)
            nonZeroInds = list(nonZeroArray[0])
            self.beta[nonZeroInds] = self.beta[nonZeroInds] * (
            (1 - 2 * self.step(1) * self.mu) ** (iteration - self.last_update[nonZeroInds] - 1))
            pi = sigmoid(numpy.dot(self.beta, whichVector), threshold=20.0)
            self.beta[nonZeroInds] = self.beta[nonZeroInds] * (1 - 2 * self.step(1) * self.mu) + self.step(1) * (
            train_example.y - pi) * whichVector[nonZeroInds]
            self.last_update[nonZeroInds] = iteration
        return self.beta
...
if __name__ == "__main__":
    ...
    if args.ec != "rate":
        ...
        lr = LogReg(len(vocab), args.mu, lambda x: float(args.step / (log(1 + x, 10))))
    ...
    for pp in range(args.passes):
        for ii in train:
            ...
            if args.ec == "df":
                ...
                beta = lr.sg_update(ii, update_number)

            if update_number % 5 == 1:
                train_lp, train_acc = lr.progress(train)
                ho_lp, ho_acc = lr.progress(test)
                train_score.append(train_acc)
                test_score.append(ho_acc)
    ...
    print("Update %i\tTP %f\tHP %f\tTA %f\tHA %f" %
          (update_number, train_lp, ho_lp, train_acc, ho_acc))
    #
    # scipy.io.savemat('/home/rohan/fall2017/726/homeworks/0mu0rate0df0lazy_train.mat', mdict={'train': train_score})
    # scipy.io.savemat('/home/rohan/fall2017/726/homeworks/0mu0rate0df0lazy_test.mat', mdict={'test': test_score})
    # # Plotting

    print(vocab[numpy.argsort(beta)[-2]])
    # print(vocab[list(beta).index(max(beta))])
    # print(vocab[list(beta).index(min(beta))])