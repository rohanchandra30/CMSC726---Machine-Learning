# Submitted by Rohan Chandra as part of CMSC 726, Homework 2.
...
class LogReg:
    ...
    def sg_update(self, train_example, iteration, lazy=False, use_tfidf=False):
        ...
        # Flag to choose whether to use lazy or not
        if lazy == False:
            pi = sigmoid(numpy.dot(self.beta, whichVector), threshold=  20.0)
            self.beta = self.beta*(1 - 2* self.step(iteration)*self.mu) + self.step(iteration)*(train_example.y - pi)*whichVector
            ...
            self.beta[nonZeroInds] = self.beta[nonZeroInds] * ( (1 - 2 * self.step(1)*self.mu)**(iteration - self.last_update[nonZeroInds] - 1))
            pi = sigmoid(numpy.dot(self.beta, whichVector), threshold=20.0)
            self.beta[nonZeroInds] = self.beta[nonZeroInds]*(1 - 2 * self.step(1)*self.mu) + self.step(1) * (train_example.y - pi) * whichVector[ nonZeroInds]
