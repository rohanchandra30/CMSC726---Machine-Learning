...
class Knearest:
    ...
    def majority(self, item_indices):
        ...
        # Storing most common elements and their frequencies...
        mostCommon = collections.Counter(item_indices).most_common()
        # Returning the most occurring label in the k nearest neighbors
        return numpy.median([self._y[mostCommon[item][0]] for item in range(len(mostCommon))])

    def classify(self, example):

        # Querying the tree and collecting k closest distances and their indices
    ...
    def confusion_matrix(self, test_x, test_y):
        ...
        d = defaultdict(dict)
        predictedLabels = [self.classify(q) for q in test_x]
        for a,b in zip(test_y, predictedLabels):
            if b not in d[a]:
                d[a][b] = 1
            else:
                d[a][b] += 1
