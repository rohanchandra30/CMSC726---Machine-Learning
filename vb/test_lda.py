...
from scipy.special import polygamma as derivdigam
...
class VariationalBayes:
    ...
    def init(self, corpus, vocab, num_topics=5, alpha=0.1):
        ...
        self._alpha = alpha
        self.ualpha = not numpy.isscalar(self._alpha)
    ...
    def e_step(self, local_parameter_iteration=50):
        ...
        if self.ualpha:
            ...
            for i in range(number_of_documents):
                alph = (self._alpha + float(self._num_types) / float(self._num_topics))
                gamma[i] *= alph
        else:
            gamma = numpy.ones((number_of_documents, self._num_topics)) * \
        ...
        for doc_id in numpy.random.permutation(number_of_documents):
            ...
            if self.ualpha:
                alph = self._alpha + float(total_word_count)/float(self._num_topics)
                gamma[doc_id, :] = alph
            else:
    ...
    def update_alpha(self, current_alpha=None, gamma=None):
        ...
        if self.ualpha:
            ...
            derivdigam(1, (self._num_topics) * current_alpha) - derivdigam(1, current_alpha))
            psi_gamma = digam(gamma)
            second_term = numpy.subtract(psi_gamma, digam(gamma.sum(axis=1))[:, None])
            ...
            (self._num_topics) * (derivdigam(2, (self._num_topics) * current_alpha)) - (derivdigam(2, current_alpha)))
...
if __name__ == "__main__":
    ...
    vb = VariationalBayes()
    if flags.update_alpha:
        vb.init(open(flags.documents), open(flags.vocab), flags.num_topics, numpy.repeat((flags.alpha),flags.num_topics))
    else:
        vb.init(open(flags.documents), open(flags.vocab), flags.num_topics, flags.alpha)
