...
class VariationalBayes:
    ...
    def update_alpha(self, current_alpha=None, gamma=None):
        ...
        # K = self._num_topics
        # Vectorize alpha
        # current_alpha = current_alpha*numpy.ones((K,1))

        # For each row of gamma matrix:
        # digam(numpy.sum(current_alpha))*numpy.ones((K,1)) - (digam(current_alpha))  + VEC_i^K(digam(gamma) - digam(sum(gamma)))
